<template>
  <div class="sc-message-list" ref="scrollList" :style="{backgroundColor: colors.messageList.bg}">
    <Message v-for="(message, idx) in messages" :message="message" :key="idx" :colors="colors" :messageStyling="messageStyling" />
    ...
    <!-- :chatImageUrl="chatImageUrl(message.author)" :authorName="authorName(message.author)" -->
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    shouldScrollToBottom() {
      return this.alwaysScrollToBottom || (this.$refs.scrollList.scrollTop > this.$refs.scrollList.scrollHeight - 1800)
    },
    profile(author) {
